...
# 앞의 두 매개변수는 창의 x, y 위치를 결정하고, 뒤의 두 매개변수는 각각 창의 너비와 높이를 결정합니다.
        ...
        btn_run = QPushButton('5.부분화면 스크린샷 실행', self)
        ...
        btn_run_fullscreen = QPushButton('6.전체화면 스크린샷 실행', self)
        btn_run_fullscreen.setToolTip('전체 화면이 찍힙니다. 다른 프로그램이 이미지를 가리지 않도록 해주세요.')
        btn_run_fullscreen.move(30, 300)
        btn_run_fullscreen.resize(btn_run_fullscreen.sizeHint())
        btn_run_fullscreen.clicked.connect(self.get_fullscreen_picture)
    ...
    def get_fullscreen_picture(self):
        rest_of_percent = 40//page
        for i in range(page):
            # if len(picture_size) >= 4:
                # pyautogui.screenshot("%s.png" % i, region=(picture_size[0], picture_size[1],
